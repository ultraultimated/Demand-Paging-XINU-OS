...
*/
	...
	*( (int *)pptr->pbase ) = MAGIC;
	...
	while(ii < 4){
		int avil;
                get_frm(&avil);
		glb_pt = (pt_t *)((FRAME0 + avil) * NBPG);
                frm_tab[avil].fr_pid = NULLPROC;
		frm_tab[avil].fr_vpno = avil + 1024;
		frm_tab[avil].fr_status = FRM_MAPPED;
                frm_tab[avil].fr_type = FR_TBL;
		jj = 0;
		while(jj < 1024) {
                        glb_pt->pt_base = ((ii * 1024) + jj);
			glb_pt->pt_write = 1;
			glb_pt->pt_pres = 1;
                        glb_pt->pt_user = 0;
                        glb_pt->pt_global = 1;
	...
	int aa = create_pd(NULLPROC);
	proctab[NULLPROC].pdbr =  (FRAME0 + aa) * NBPG;
...
int create_pd(int pid){
		...
		int init = 0;
		if(!init){
			glb_ptt = (pd_t *)((FRAME0 + avil) * NBPG);
			frm_tab[avil].fr_type = FR_DIR;
			frm_tab[avil].fr_vpno = avil + 1024;
			frm_tab[avil].fr_status = FRM_MAPPED;
			frm_tab[avil].fr_pid = pid;
			init = 1;
		}
		do{
			if(jj >= 4){
				...
				glb_ptt->pd_base = jj + 1024;
                                glb_ptt->pd_avail = 0;
                                glb_ptt->pd_pres = 1;                                                                                                         glb_ptt->pd_global = 0;
                                glb_ptt->pd_fmb = 0;
                                glb_ptt->pd_acc = 0;
                                glb_ptt->pd_mbz = 0;                                                                                                          glb_ptt->pd_pcd = 0;
                                glb_ptt->pd_pwt = 0;
                                glb_ptt->pd_write = 1;
                                glb_ptt->pd_user = 0;
                                glb_ptt++;
